...
class SimpleButton  {
  ...
  const char * codeToString(ReturnCode code) const { 
    ...
    switch (code){
    case ReturnCode::ok: return PSTR("ok");
      break;
    case ReturnCode::notNeeded: return PSTR("notNeeded");
      break; 
    case ReturnCode::isPolling: return PSTR("isPolling");
      break;
    case ReturnCode::noChat: return PSTR("noChat");
      break;
    case ReturnCode::noMesgId: return PSTR("noMesgId");
      break;
    case ReturnCode::wrongResponse: return PSTR("wrongResponse");
      break;
    }
    return PSTR("?");
  }
...
const ReturnCode cleaner( const bool waitBotResponse=false){
  if ( ! menuIds.hasMenuId( this->chatId() ) ) return ReturnCode::noMesgId;
...
//CREATOR
  ...
  const ReturnCode updater( const bool waitBotResponse=false ){
    ...
    if ( chatId() == 0 ) ret = ReturnCode::noChat;
      ...
      fb::InlineMenu menu( buttonName(), cmd.c_str() );
  ...
  //   if ( waitBotResponse ){        
  //     //builtInLed.flashOff();
  //     fb::Result res = botP->editText( text, waitBotResponse /*true*/ );
  //     if( ! res.valid()) {
